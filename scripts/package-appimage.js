...
async function buildAppImage(options){const appDir=path.resolve(options.appDir),identity=releaseIdentity(options.version,options.wrapperRevision??null),version=identity.factoryVersion;assertPackagedRuntimeLayout(appDir);assertNoBundledDroid(appDir);assertAcceptedPatchReport(appDir);validateAppJavaScript(appDir);scanPackageTree(appDir,{workspaceRoot:process.cwd()});
  ...
  fs.copyFileSync(path.resolve("packaging/appimage/factory-desktop.desktop"),path.join(appDirRoot,"factory-desktop.desktop"));fs.copyFileSync(path.resolve("assets/factory-desktop.svg"),path.join(appDirRoot,"factory-desktop.svg"));const iconSource=path.join(appDir,"resources","factory-desktop.png");if(!fs.statSync(iconSource,{throwIfNoEntry:false})?.isFile())throw new Error(`Staged app is missing required factory-desktop.png icon: ${iconSource}`);fs.copyFileSync(iconSource,path.join(appDirRoot,"factory-desktop.png"));fs.chmodSync(path.join(appDirRoot,"factory-desktop.png"),0o644);
