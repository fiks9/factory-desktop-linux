...
function buildRpm(options) {
  ...
  const appDir = path.resolve(options.appDir), version = options.version;
  if (!/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(version)) throw new Error(`Invalid RPM version: ${version}`);
  assertPackagedRuntimeLayout(appDir); assertNoBundledDroid(appDir); assertAcceptedPatchReport(appDir);
